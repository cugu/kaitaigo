...
func TestGPT(t *testing.T) {
	...
	primary := gpt.Primary()
	assert.EqualValues(t, &runtime.ByteArray{Size: 8, Data: []uint8{0x45, 0x46, 0x49, 0x20, 0x50, 0x41, 0x52, 0x54}}, primary.Signature())
	partitions := primary.Entries()[0]
	name := *partitions.Name()
	u16 := []uint16{}
	for i := 0; i < len(name.Data); i += 2 {
		u16 = append(u16, binary.LittleEndian.Uint16(name.Data[i:i+2]))
	}
